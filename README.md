...
2. **Semantic alignment-gated convergence.** After each debate round the evolving solution is embedded (`all-MiniLM-L6-v2`) and compared to the original problem statement by **cosine similarity**. If the score falls below a configurable threshold (default `0.75`), another debate round is triggered — capped at **3 rounds** so it can never loop forever. No LLM is used to judge similarity.
   ...
   Alongside the gate, **Step 5 (Theme & Problem Alignment)** produces a user-facing **/100 breakdown** across five categories — theme relevance, problem relevance, user–problem fit, solution relevance, and feasibility — plus per-category evidence and, where a category scores below 75, a specific improvement suggestion. Scoring is **directional** (how much of the theme/problem's meaning is actually covered by the selected idea, with light stemming), evaluated against *all* of the idea's fields, and blended with the semantic cosine — so a genuinely well-aligned project lands in the 75–90 band while an off-theme or vague idea stays low. Feasibility factors the timeline agent's estimated build hours against the entered time limit, so an over-scoped project is penalised. Before scoring, a single **gated** strengthening pass may make an *already-present* theme/problem link explicit in the idea's own text (never adding features, swapping the idea, or lifting an unrelated one). `overall` is strictly the mean of the five categories. All deterministic — no LLM, no hard-coded or inflated numbers.
